<?php
/**
 * Template part for displaying page content in page.php
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/
 *
 * @package cjt
 */

?>

<section class="lg:px-24">
	<div class="lg:container lg:mx-auto grid grid-cols-2 lg:gap-x-12 items-center">
		<div>

			<h1 class="text-7xl font-display leading-tight">
				The perfect WordPress plugin for adding code
			</h1>
			<h2 class="text-3xl font-display leading-none tracking-widest mt-5">
				Modular. Versatile. Effortless
			</h2>
			<p class="mt-10 max-w-screen-sm text-neutral-600">Supercharge Your WordPress Experience! Meet the ultimate code-adding plugin for effortless customization. From rookies to coding ninjas, unleash your website's full potential now.</p>

			<a href="/" class="bg-brand-blue rounded mt-10 border border-brand-blue/70 px-8 py-3 shadow-md text-white text-xl inline-flex justify-center items-center">Get CJT PLUS NOW</a>
		</div>
		<div class="relative">
			<svg viewBox="0 0 417 417" fill="none" xmlns="http://www.w3.org/2000/svg">
				<path d="M164 360.5C139.667 342.333 98.3002 296.3 127.5 257.5C164 209 285.5 301.5 289 194.5C292.5 87.5 165 184.5 178.5 54M416 208.5C416 323.099 323.099 416 208.5 416C93.9009 416 1 323.099 1 208.5C1 93.9009 93.9009 1 208.5 1C323.099 1 416 93.9009 416 208.5Z" stroke="#005DA4" />
			</svg>


			<div class="rounded-md shadow-md px-5 py-2 absolute bg-white border border-neutral-950 top-10 -left-4">
				<div class="flex flex-col items-center justify-center">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="0.5" stroke="currentColor" class="size-14">
						<path stroke-linecap="round" stroke-linejoin="round" d="M12 9.75v6.75m0 0-3-3m3 3 3-3m-8.25 6a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.233-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z" />
					</svg>
					<p class="text-3xl font-bold leading-loose tracking-wider font-display">10,000+</p>
					<span class="text-sm tracking-wide">Downloads</span>

				</div>
			</div>

			<div class="rounded-md shadow-md px-5 py-2 absolute bg-white border border-neutral-950 bottom-10 right-4">
				<div class="flex flex-col items-center justify-center space-y-3">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="0.5" stroke="currentColor" class="size-14">
						<path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
					</svg>

					<p class="text-3xl font-bold leading-none tracking-wider font-display">
						4.9<small class="text-xs font-normal font-body">/5</small>
					</p>
					<div class="flex items-center gap-x-2 mb-3">
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
							<path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
						</svg>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
							<path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
						</svg>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
							<path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
						</svg>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
							<path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
						</svg>
						<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
							<path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clip-rule="evenodd" />
						</svg>
					</div>

					<span class="text-sm tracking-wide">1000+ verified reviews</span>
				</div>
			</div>
		</div>
	</div>
</section>
